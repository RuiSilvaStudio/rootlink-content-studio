...
export function themeCssVars(theme: Theme | null | undefined, mode: ThemeMode): Record<string, string> {
  ...
  '--cs-font-family': theme?.fontBody ? (typeof theme.fontBody === 'object' ? `${theme.fontBody.family}, ${theme.fontBody.fallback}` : 'Inter') : 'Inter',
  '--cs-font-mono': theme?.fontMono ? (typeof theme.fontMono === 'object' ? `${theme.fontMono.family}, ${theme.fontMono.fallback}` : 'JetBrains Mono') : 'JetBrains Mono',
